...
- 分类、广告标识、直播硬拦截和推广过滤
- 分享规则文件、CLI 覆盖与分享候选判定
- 评论策略和状态机
- 扫描 GUI 命令生成，以及默认不执行真实评论/分享的副作用开关
- 扫描运行日志中的安全门原因
- README 与交接文档的关键安全承诺

这些测试不访问真实站点，也不会打开浏览器、调用 LLM、发送评论或分享内容。真实站点回归需在已登录的独立环境中手动执行；先使用 [8.2](#82-安全扫描搞笑候选推荐起步) 的安全扫描命令，小轮次确认日志后才考虑启用真实分享。
